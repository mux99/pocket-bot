...
<div class="min-h-screen flex items-center justify-center bg-gray-900 text-white">
    ...
    <div class="bg-gray-800 p-6 rounded shadow-lg max-w-md w-full">
        <form action="?/edit" method="POST" class="inline-block mr-2">
            ...
            <p class="mb-2"><span class="font-semibold">Winner:</span> <input id="winner_id" name="winner_id" type="number" class="border rounded px-2 mb-2 text-black" bind:value={data.part.winner_user_id} style="width: 50px;" /> (<a href="/admin/users/{data.part.winner_user_id}">{data.part.winner_username}</a>)</p>
            <p class="mb-2"><span class="font-semibold">Loser:</span> <input id="loser_id" name="loser_id" type="number" class="border rounded px-2 mb-2 text-black" bind:value={data.part.loser_user_id} style="width: 50px;" /> (<a href="/admin/users/{data.part.loser_user_id}">{data.part.loser_username}</a>)</p>
